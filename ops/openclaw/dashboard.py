            ...
            with analysis_area:
                with st.expander(f"📌 Analiz ve Teklif: {comp}", expanded=True):
                    ...
                    with st.spinner("🤖 Derin analiz yapılıyor..."):
                        ...
                        ai_ana, info = call_llm_raw(messages, mode=m_str, gateway_pw=g_pw, timeout=60)
                        ...
                        if company_url:
                            col3.markdown(f'<a href="{company_url}" target="_blank" style="display:block;text-align:center;background:#EC6602;color:white;padding:6px 12px;border-radius:8px;text-decoration:none;font-weight:600;">Siteye Git</a>', unsafe_allow_html=True)
